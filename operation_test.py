...
# op = Translate(image=np.ones((20,10),dtype=np.float32),translate=[1,1])
# output = op.run()
# print(f"Translate Output \n {output}")


image = cv2.imread("tests\data\grayscale_dog.jpg",cv2.IMREAD_UNCHANGED)
op = Translate(image=image.astype(np.float32),translate=[590,-40],fillMode=FillMode.REFLECTION)
output = op.run()
