...
extern "C" __global__ 
void rotate(float* image,float* out, float theta, float* _pivot, unsigned int* dims, unsigned int fillMode)
{
 ...
 if (dstx >= width || dsty >= height)
  ...
  if(channels == 3){
    ...
    out3c[outIdx] = sample2d<float3>(image3c,srcPoint.x,srcPoint.y,dims,fillMode,INTERPOLATION_MODE_POINT,make_float3(0.0f,0.0f,0.0f));
  }
  else{
    out[outIdx] = sample2d<float>(image,srcPoint.x,srcPoint.y,dims,fillMode,INTERPOLATION_MODE_POINT,0.0f);
  }
